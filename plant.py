# Form implementation generated from reading ui file 'plant.ui'
#
...
class CheckProcess(QRunnable):
    ...
    @pyqtSlot()
    def run(self):
        global plant_process, is_run
        while plant_process.is_alive() and is_run:
...
class Ui_PlantUI(object):
    def setupUi(self, PlantUI, dict):
        global window, uis
        uis = dict
        window = PlantUI
        font = QtGui.QFont()
        font.setPointSize(16)
        PlantUI.setFont(font)
        PlantUI.setStyleSheet("*{\n"
                              "    border: none;\n"
                              ...
                              "    background-color: #16191d;\n"
                              "}")
        self.centralwidget = QtWidgets.QWidget(parent=PlantUI)
        ...
        self.horizontalLayout_2 = QtWidgets.QHBoxLayout(self.subconfigitem)
        ...
        self.scrollAreaWidgetContents.setGeometry(QtCore.QRect(0, 0, 458, 237))
        ...
        PlantUI.setCentralWidget(self.centralwidget)
        ...
        self.runBtn.clicked.connect(self.run_plant_farming)
        self.runBtn.setAutoDefault(True)

        self.retranslateUi(PlantUI)
        QtCore.QMetaObject.connectSlotsByName(PlantUI)
    ...
    def run_plant_farming(self):
        global plant_process, window, is_run
        # if everything is entered correctly
        if str(self.placeCombo.currentText()) != "--" and str(self.actionCombo.currentText()) != "--" and not is_run:
            # delete everything from the old log
            if os.path.isfile("log.txt"):
                os.remove("log.txt")
            is_run = True
            plant_process = multiprocessing.Process(target=run_script,
                                                    args=[str(self.placeCombo.currentText()),
                                                          str(self.actionCombo.currentText())])
        ...
        elif is_run and plant_process is not None and plant_process.is_alive():
            self.hide_status()
    ...
    def retranslateUi(self, PlantUI):
        _translate = QtCore.QCoreApplication.translate
        PlantUI.setWindowTitle(_translate("MainWindow", "PokemonUI"))
        ...
        self.title.setText(_translate("MainWindow", "Plant Farming"))
        self.placeCombo.setItemText(0, _translate("MainWindow", "--"))
        self.actionCombo.setItemText(0, _translate("MainWindow", "--"))
